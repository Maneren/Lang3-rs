fn run_optimized(source: &str) -> Vec<String> {
    l3::run_pipeline_optimized(source, "<test>").unwrap()
}
...
fn propagates_local_constant() {
    let source = "let a = 42;\nprintln(a)\n";
    assert_eq!(run_optimized(source), vec!["42"]);
...
fn folds_reassignment() {
    let source = "let a = 42;\na = a + 1;\nprintln(a)\n";
    assert_eq!(run_optimized(source), vec!["43"]);
    let bytecode = bytecode_optimized(source);
    assert!(!bytecode.contains("ADD"));
...
fn folds_string_concat_through_local() {
    ...
    assert_eq!(run_optimized(source), vec!["hello"]);
    let bytecode = bytecode_optimized(source);
    assert!(!bytecode.contains("ADD"));
...
fn preserves_closure_capture_semantics() {
    ...
    assert_eq!(run_optimized(source), vec!["6"]);
}

#[test]
fn preserves_loop_constant_invalidation() {
    let source = "let i = 0;\nwhile i < 3 do\n  i = i + 1;\nend\nprintln(i)\n";
    assert_eq!(run_optimized(source), vec!["3"]);
}

#[test]
fn removes_unreachable_code_after_return() {
    let source = "fn f(n)\n  while n > 0 do\n    n = n - 1;\n    return 5\n  end\n  return 6\nend\nprintln(f(3))\n";
    assert_eq!(run_optimized(source), vec!["5"]);
}

#[test]
fn preserves_chained_comparison() {
    let source = "let a = 1;\nlet b = 3;\nprintln(a < b and b < 5)\n";
    assert_eq!(run_optimized(source), vec!["true"]);
}

#[test]
fn preserves_parameter_alignment_in_function_chunks() {
    let source = "fn f(x)\n  let mut acc = 0\n  for i in x - 1 ..= x + 1 do\n    acc = acc + i\n  end\n  return acc\nend\nprintln(f(3))\n";
    assert_eq!(run_optimized(source), vec!["9"]);
}
