...
use l3_ast::{
    AnonymousFunction, Array, AssignmentOperator, BinaryExpression, BinaryOperator, Block, Boolean,
    BreakStatement, Comparison, ComparisonOperator, ContinueStatement, Declaration, Expression,
    Float, ForLoop, FunctionBody, FunctionCall, Identifier, IfBase, IfExpression, IfStatement,
    IndexExpression, LastStatement, Literal, LogicalExpression, LogicalOperator, Mutability,
    NameAssignment, NamedFunction, Nil, Number, OperatorAssignment, Program, RangeForLoop,
    RangeOperator, ReturnStatement, Statement, StringLiteral, UnaryExpression, UnaryOperator,
    Variable, While,
};
...
impl<'input> Parser<'input> {
    ...
    pub fn new(source: &'input str, filename: &str) -> Self {
        ...
        let peek = lexer.next().and_then(std::result::Result::ok);
        Self {
            lexer,
            peek,
    ...
    fn advance(&mut self) -> Option<(usize, Token<'input>, usize)> {
        ...
        self.peek = self.lexer.next().and_then(std::result::Result::ok);
        if current.is_none() {
            self.eof = true;
        }
        current
    ...
    fn parse_or_expr(&mut self) -> Result<Box<Expression>, String> {
        ...
        while matches!(self.peek_token(), Some(Token::Or)) {
            ...
            left = Box::new(Expression::LogicalExpression(LogicalExpression::new(
                *left,
                LogicalOperator::Or,
                *right,
                self.loc(),
    ...
    fn parse_add_expr(&mut self) -> Result<Box<Expression>, String> {
        ...
        loop {
            ...
            left = Box::new(Expression::BinaryExpression(BinaryExpression::new(
                *left,
                op,
                *right,
                self.loc(),
    ...
    fn parse_unary_expr(&mut self) -> Result<Box<Expression>, String> {
        match self.peek_token() {
            Some(Token::Minus) => {
                ...
                Ok(Box::new(Expression::UnaryExpression(UnaryExpression::new(
                    UnaryOperator::Minus,
                    *expr,
                    self.loc(),
                ))))
    ...
    fn parse_variable(&mut self) -> Result<Variable, String> {
        let name = if let Some(Token::Ident(name)) = self.peek_token().copied() {
            self.advance();
            name
        } else {
        ...
        while matches!(self.peek_token(), Some(Token::LBracket)) {
            ...
            var = Variable::IndexExpression(IndexExpression::new(var, index, self.loc()));
        }
        Ok(var)
    }

    fn parse_primary_expr(&mut self) -> Result<Box<Expression>, String> {
        match self.peek_token().copied() {
            ...
            Some(Token::Nil) => {
                self.advance();
                Ok(Box::new(Expression::Literal(Literal::Nil(Nil::new(
                    self.loc(),
                )))))
            }
            Some(Token::True) => {
                self.advance();
                Ok(Box::new(Expression::Literal(Literal::Boolean(
                    Boolean::new(true, self.loc()),
                ))))
            }
            Some(Token::False) => {
                self.advance();
                Ok(Box::new(Expression::Literal(Literal::Boolean(
                    Boolean::new(false, self.loc()),
                ))))
            }
            Some(Token::Number(n)) => {
                self.advance();
                if matches!(self.peek_token(), Some(Token::Dot)) {
                    self.advance();
                    if let Some(Token::Number(f)) = self.peek_token().copied() {
                        self.advance();
                        let val = format!("{n}.{f}").parse::<f64>().unwrap();
                        Ok(Box::new(Expression::Literal(Literal::Float(Float::new(
                            val,
                            self.loc(),
                        )))))
                    } else {
                    ...
                    Ok(Box::new(Expression::Literal(Literal::Number(Number::new(
                        n,
                        self.loc(),
                    )))))
                }
            ...
            Some(Token::Str(s)) => {
                ...
                Ok(Box::new(Expression::Literal(Literal::String(
                    StringLiteral::new(unescaped, self.loc()),
                ))))
            }
            Some(Token::Ident(_)) => {
                ...
                if matches!(self.peek_token(), Some(Token::LParen)) {
                    let (name, _) = match &var {
                        ...
                        Variable::IndexExpression(_) => (String::new(), false),
                    };
                    if matches!(var, Variable::Identifier(_)) {
                        let args = self.parse_call_args()?;
                        Ok(Box::new(Expression::FunctionCall(FunctionCall::new(
                            self.mk_id(&name),
                            args,
                            self.loc(),
                        ))))
            ...
            Some(Token::Return) => Err("unexpected return in expression context".to_string()),
            _ => Err(format!(
    ...
    fn parse_array_lit(&mut self) -> Result<Box<Expression>, String> {
        ...
        self.expect(Token::RBracket)?;
        Ok(Box::new(Expression::Literal(Literal::Array(Array::new(
            elements,
            self.loc(),
        )))))
    }
    ...
    fn parse_if_expr(&mut self) -> Result<Box<Expression>, String> {
        ...
        Ok(Box::new(Expression::IfExpression(IfExpression::new(
    ...
    fn parse_anon_fn(&mut self) -> Result<Box<Expression>, String> {
        ...
        Ok(Box::new(Expression::AnonymousFunction(
            AnonymousFunction::new(FunctionBody::new(params, body, self.loc()), self.loc()),
        )))
    }
    ...
    fn parse_params(&mut self) -> Result<Vec<Identifier>, String> {
        ...
        if matches!(self.peek_token(), Some(Token::Ident(_))) {
            if let Some(Token::Ident(name)) = self.peek_token().copied() {
                self.advance();
    ...
    fn parse_statement(&mut self) -> Result<Statement, String> {
        match self.peek_token().copied() {
            ...
            Some(Token::Return | Token::Break | Token::Continue) => unreachable!(), // handled by parse_last_statement
            Some(Token::Ident(_)) => {
        ...
        match self.peek_token() {
            Some(
                ...
                if op == AssignmentOperator::Assign && is_simple_ident {
                    let Variable::Identifier(name) = var else {
                        unreachable!()
                    };
                    Ok(Statement::NameAssignment(NameAssignment::new(
                        vec![name],
                        expr,
                        self.loc(),
                    )))
                } else {
                    Ok(Statement::OperatorAssignment(OperatorAssignment::new(
                        var,
                        op,
                        expr,
                        self.loc(),
    ...
    fn parse_while(&mut self) -> Result<Statement, String> {
        ...
        let body = self.parse_block()?;
        self.expect(Token::End)?;
        Ok(Statement::While(While::new(cond, body, self.loc())))
    }
    ...
    fn parse_for(&mut self) -> Result<Statement, String> {
        ...
        let var = if let Some(Token::Ident(name)) = self.peek_token().copied() {
            self.advance();
            self.mk_id(name)
        } else {
        ...
        if matches!(self.peek_token(), Some(Token::DotDot | Token::DotDotEqual)) {
            ...
            let body = self.parse_block()?;
            self.expect(Token::End)?;
            Ok(Statement::RangeForLoop(RangeForLoop::new(
                var,
                start,
                end,
                step,
    ...
    fn parse_fn_def(&mut self) -> Result<Statement, String> {
        ...
        Ok(Statement::NamedFunction(NamedFunction::new(
            name,
    ...
    fn parse_last_statement(&mut self) -> Result<LastStatement, String> {
        match self.peek_token() {
            Some(Token::Return) => {
                ...
                let expr = if matches!(
                    self.peek_token(),
                    Some(Token::End | Token::Else | Token::Elif)
                ) {
                    None
                } else {
                    Some(*self.parse_expr()?)
                };
                Ok(LastStatement::Return(ReturnStatement::new(
                    expr,
                    self.loc(),
                )))
            }
            Some(Token::Break) => {
                self.advance();
                Ok(LastStatement::Break(BreakStatement::new(self.loc())))
            }
            Some(Token::Continue) => {
                self.advance();
                Ok(LastStatement::Continue(ContinueStatement::new(self.loc())))
            }
    ...
    fn parse_block(&mut self) -> Result<Block, String> {
        ...
        loop {
            match self.peek_token() {
                Some(Token::End | Token::Else | Token::Elif) | None => break,
                Some(Token::Return | Token::Break | Token::Continue) => {
