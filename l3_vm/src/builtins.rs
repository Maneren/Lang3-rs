...
pub fn builtins() -> Vec<(&'static str, Builtin)> {
    vec![
                ...
                |args: Vec<StackValue>, heap: &mut Heap| -> Result<StackValue, RuntimeError> {
                    ...
                    match sv {
                        StackValue::Heap(key) => {
                            if let Some(cell) = heap.cells.get(*key) {
                                match &cell.value {
                                    HeapData::String(s) => Ok(StackValue::Primitive(
                                        Primitive::Integer(s.len() as i64),
                                    )),
                                    HeapData::Vector(v) => Ok(StackValue::Primitive(
                                        Primitive::Integer(v.len() as i64),
                                    )),
                            ...
                            if let Some(cell) = heap.cells.get(*key) {
                                match &cell.value {
                                    ...
                                    _ => Err(RuntimeError::type_error(
                                        "head requires a vector or string",
                                    )),
                                }
                            } else {
                        ...
                        StackValue::Heap(key) => {
                            if let Some(cell) = heap.cells.get(*key) {
                                match &cell.value {
                                    ...
                                    _ => Err(RuntimeError::type_error(
                                        "tail requires a vector or string",
                                    )),
                                }
                            } else {
                                Err(RuntimeError::type_error("invalid heap reference"))
                        ...
                        StackValue::Heap(key) => {
                            if let Some(cell) = heap.cells.get(*key) {
                                match &cell.value {
                                    ...
                                    _ => Err(RuntimeError::type_error(
                                        "drop requires a vector or string",
                                    )),
                                }
                            } else {
                                Err(RuntimeError::type_error("invalid heap reference"))
                        ...
                        StackValue::Heap(key) => {
                            if let Some(cell) = heap.cells.get(*key) {
                                match &cell.value {
                                    ...
                                    _ => Err(RuntimeError::type_error(
                                        "take requires a vector or string",
                                    )),
                                }
                            } else {
                                Err(RuntimeError::type_error("invalid heap reference"))
        ...
        (
            "id",
            Rc::new(
                |args: Vec<StackValue>, _heap: &mut Heap| -> Result<StackValue, RuntimeError> {
        ...
        (
            "input",
            Rc::new(
                |_: Vec<StackValue>, heap: &mut Heap| -> Result<StackValue, RuntimeError> {
